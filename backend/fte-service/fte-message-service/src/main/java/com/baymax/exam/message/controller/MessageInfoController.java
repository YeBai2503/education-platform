...
//    MessageWebSocketService messageWebSocketService;
    ...
    @Inner
    @Operation(summary = "发送消息(MessageInfo)")
    @PostMapping("/inner/send-message-info")
    public Boolean sendMessage(@RequestBody @Validated MessageInfo messageInfo){
        // 将MessageInfo转换为MessageResult
        MessageResult messageResult = new MessageResult();
        messageResult.setInfo(messageInfo);
        messageResult.setCode(MessageCodeEnum.HOMEWORK_CONSOLE_STATISTICS);
        
        return sendMessage(messageResult);
    }
