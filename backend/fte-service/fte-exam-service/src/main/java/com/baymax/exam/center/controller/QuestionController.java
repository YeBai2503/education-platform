...
public class QuestionController {
    ...
    public Result<Question> getQuestionById(@PathVariable("id") Integer id) {
        ...
        if (question == null) {
            return Result.failed(ResultCode.PARAM_ERROR, "题目不存在");
        }
        return Result.success(question);
    }
    
    @Operation(summary = "根据ID批量获取题目")
    @GetMapping("/listByIds")
    public Result<List<Question>> getQuestionsByIds(@RequestParam("ids") List<Integer> ids) {
