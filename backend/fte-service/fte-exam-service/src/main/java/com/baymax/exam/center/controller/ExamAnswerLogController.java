...
public class ExamAnswerLogController {
    ...
    @Operation(summary = "批量查询考试状态")
    @PostMapping("/batch-status")
    public Result<Map<Integer, String>> batchQueryExamStatus(@RequestBody List<Integer> examIds) {
        Integer userId = UserAuthUtil.getUserId();
        Map<Integer, String> statusMap = new HashMap<>();
        
        for (Integer examId : examIds) {
            // 查询最新的提交记录
            List<ExamAnswerLogEnum> statusList = List.of(
                ExamAnswerLogEnum.SUBMIT, 
                ExamAnswerLogEnum.ROBOT_REVIEW, 
                ExamAnswerLogEnum.TEACHER_REVIEW
            );
            
            LambdaQueryWrapper<ExamAnswerLog> queryWrapper = new LambdaQueryWrapper<>();
            queryWrapper.eq(ExamAnswerLog::getExamId, examId)
                    .eq(ExamAnswerLog::getStudentId, userId)
                    .in(ExamAnswerLog::getStatus, statusList)
                    .orderByDesc(ExamAnswerLog::getCreatedAt)
                    .last("LIMIT 1");
            
            ExamAnswerLog log = examAnswerLogService.getOne(queryWrapper);
            
            if (log != null) {
                // 转换状态为中文
                statusMap.put(examId, log.getStatus().getAction());
            } else {
                // 未提交
                statusMap.put(examId, "未提交");
            }
        }
