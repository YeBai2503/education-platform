<template>
  <div class="project-submit">
    <a-card class="general-card" :title="isEdit ? '修改项目提交' : '提交项目'">
      <!-- 团队信息 -->
      <a-card class="inner-card" v-if="teamInfo" style="margin-bottom: 20px">
        <template #title>
          团队信息
          <a-tag color="orange" style="margin-left: 8px">只有队长可以提交</a-tag>
        </template>
        <a-descriptions :data="teamInfoDescData" layout="inline-vertical" :column="3" />
      </a-card>
      ...
      <a-form
        ...
        :style="{ width: '600px' }"
      >
        <a-form-item field="detail" label="项目提交说明" extra="请输入详细的项目提交说明，此项为必填">
          <a-textarea
            v-model="formData.detail"
            placeholder="请输入项目提交说明"
            allow-clear
  ...
  setup() {
            ...
            if (teamInfo.value) {
              ...
              if (headerRes.data.code === '00000') {
                ...
                if (!isTeamHeader.value) {
                  Message.error('只有团队队长才能提交项目');
                  goBack();
                  return false;
                }
              } else {
                Message.error(headerRes.data.msg || '验证队长权限失败');
                goBack();
                return false;
              }
            }
          } else {
            Message.error(teamRes.data.msg || '获取团队信息失败');
            goBack();
            return false;
          }
        } else {
          Message.error('您未加入任何团队，请先加入团队');
          goBack();
          return false;
        }
        return true;
    ...
    const submitForm = async () => {
      ...
      try {
        ...
        if (res.data.code === '00000') {
          Message.success(isEdit.value ? '更新项目提交成功' : '提交项目成功');
          // 刷新提交信息
          ...
          Message.error(res.data.msg || (isEdit.value ? '更新项目提交失败' : '提交项目失败'));
        }
      } catch (error) {
        console.error('提交出错:', error);
        Message.error(isEdit.value ? '更新项目提交失败' : '提交项目失败');
      } finally {
    ...
    const goBack = () => {
      router.push({
        name: 'ProjectDetail',
        params: { courseId: courseId.value, experimentId: experimentId.value }
      });
    };
    ...
    const teamInfoDescData = computed(() => {
      ...
      return [
        ...
        {
          label: '成员数',
          value: `${teamInfo.value.memberCount || 0} / 5`
        }
      ];
    });
    ...
    onMounted(async () => {
      const isValid = await checkTeamStatus();
      if (isValid) {
