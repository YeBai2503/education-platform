<template>
  <div class="project-create">
    <a-card class="general-card" :title="isEdit ? '编辑项目实训' : '创建项目实训'">
      <a-form
        ref="formRef"
        :model="formData"
        label-align="left"
        :style="{ width: '600px' }"
      >
        <a-form-item field="title" label="项目标题" required extra="项目标题为必填项">
          <a-input
            v-model="formData.title"
            placeholder="请输入项目实训标题"
            allow-clear
          />
        </a-form-item>
        <a-form-item field="detail" label="项目详情" extra="请输入项目实训的详细描述">
          <a-textarea
            v-model="formData.detail"
            placeholder="请输入项目实训详情"
            allow-clear
            :auto-size="{ minRows: 5, maxRows: 10 }"
          />
        </a-form-item>
        <a-form-item field="ddl" label="截止日期" extra="设置学生提交项目实训的截止日期">
          <a-date-picker
  ...
  setup() {
    ...
    const submitForm = async () => {
      ...
      try {
        ...
        console.log(res);
        if (res.data.code === '00000') {
          Message.success(isEdit.value ? '更新项目实训成功' : '创建项目实训成功');
          goBack();
        } else {
          Message.error(res.data.msg || (isEdit.value ? '更新项目实训失败' : '创建项目实训失败'));
        }
      } catch (error) {
        console.error('提交出错:', error);
        Message.error(isEdit.value ? '更新项目实训失败' : '创建项目实训失败');
      } finally {
