...
// 查询考试提交状态
export const getExamSubmitStatusRequest=(examInfoIds)=>{
    console.log('发送考试状态查询请求，参数:', examInfoIds);
    return axios.post(`/eapi/exam-answer-log/batch-status`, examInfoIds)
}
// 获取学生的所有考试
export const getStudentExamsRequest=(status=0, page=1, pageSize=10)=>{
    return axios.get(`/eapi/exam-info/list?status=${status}&page=${page}&pageSize=${pageSize}&skipClassCheck=true`)
}
/**---------------------考试控制台啦~相关-----------------------------*/
